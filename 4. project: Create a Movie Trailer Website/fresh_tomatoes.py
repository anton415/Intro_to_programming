...
def create_movie_tiles_content(movies):
    ...
    for movie in movies:
        ...
        if movies[movie]['type'] == 'movie' :
            youtube_id_match = re.search(r'(?<=v=)[^&#]+', movies[movie]['trailer_youtube_url'])
            youtube_id_match = youtube_id_match or re.search(r'(?<=be/)[^&#]+', movies[movie]['trailer_youtube_url'])
            trailer_youtube_id = youtube_id_match.group(0) if youtube_id_match else None

            # Append the tile for the movie with its content filled in
            content += movie_tile_content.format(
                movie_title=movies[movie]['title'],
                poster_image_url=movies[movie]['poster_image_url'],
                trailer_youtube_id=trailer_youtube_id
            )
        if movies[movie]['type'] == 'shows' :
            youtube_id_match = re.search(r'(?<=v=)[^&#]+', movies[movie]['trailer_youtube_url'])
            youtube_id_match = youtube_id_match or re.search(r'(?<=be/)[^&#]+', movies[movie]['trailer_youtube_url'])
            trailer_youtube_id = youtube_id_match.group(0) if youtube_id_match else None
